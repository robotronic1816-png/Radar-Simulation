...
def cfar_2d_fast(rd_db, tr=12, gr=6, td=8, gd=4, offset_db=10.0):
    """
    Vectorized CA-CFAR: 100x faster.
    """
    d_win = 2 * (td + gd) + 1
    r_win = 2 * (tr + gr) + 1
    mask = np.ones((d_win, r_win))
    
    # Zero out guard + CUT
    center_d = td + gd
    center_r = tr + gr
    mask[center_d - gd : center_d + gd + 1, center_r - gr : center_r + gr + 1] = 0
    ...
    noise_lin = convolve(rd_lin, mask, mode='constant', cval=np.mean(rd_lin))
    noise_db = 10 * np.log10(noise_lin + 1e-9)
    
    return (rd_db > (noise_db + offset_db)).astype(int)
# RADAR PARAMETERS
c = 3e8
fc = 77e9
B = 200e6
